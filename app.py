...
_SOURCE_TOKS = ["REMUX", "BLURAY", "BLU-RAY", "BD", "WEB-DL", "WEBDL", "WEBRIP", "HDTV", "DVDRIP", "CAM", "TS"]
...
def filter_and_format(type_: str, id_: str, streams: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], PipeStats]:
    ...
    for s in streams:
        ...
        if m['seeders'] < MIN_SEEDERS:
            continue  # Drop low seeders
        if m['language'] != PREFERRED_LANG:
            continue  # Prefer EN lang
    ...
    # Sort by priority: cached > seeders > premium provider
    priority_order = {'RD': 3, 'TB': 2, 'AD': 1, 'AIOStreams': 0, 'UNK': 0}
    out_pairs.sort(key=lambda p: (cached_map.get(p[1].get('infohash', ''), False), p[1].get('seeders', 0), priority_order.get(p[1].get('provider', 'UNK'), 0)), reverse=True)
...
def stream(type_: str, id_: str):
    ...
    try:
        ...
        logger.info(
            "WRAP_STATS rid=%s type=%s id=%s aio_in=%s merged_in=%s dropped_error=%s dropped_missing_url=%s dropped_pollution=%s deduped=%s delivered=%s ms=%s",
            _rid(), type_, id_,
