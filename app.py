...
# ---------------------------
# Config (format-testing defaults)
# ---------------------------
AIO_BASE = os.environ.get("AIO_BASE", "").rstrip("/")  # e.g. https://aiostreams.example.com
...
def _ui_clean(t: str, max_len: int) -> str:
    ...
    for k, v in _PUNCT_MAP.items():
        t = t.replace(k, v)
    # Preserve emojis by not stripping them globally; we only clean obvious file-like junk.
    t = _ascii_fold(t)
...
def classify(s: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Size (in bytes; keep also a UI string)
    size_bytes = 0
    ...
    # Source / codec from name (best-effort)
    up = (name or "").upper()
    source = ""
    ...
    # Seeders: not present in your sample file; default 0
    seeders = 0

    # Infohash (magnet or query/path 40-hex)
    infohash = None
    u2 = (s.get("url") or "")
    parsed = urlparse(u2)
    if "magnet:" in u2:
    ...
    # Title candidate (strip obvious technical prefixes)
    title_raw = name or ""
    ...
    return {
        ...
        "codec": codec,
        "title_raw": title_raw,
        "lang": "EN",  # placeholder; extend later
    }
...
def get_expected_metadata(type_: str, id_: str) -> Dict[str, Any]:
    # TODO: plug Trakt/TMDB later; keep stable contract now
    return {
...
def validate_stream(_s: Dict[str, Any], _meta: Dict[str, Any], _expected: Dict[str, Any]) -> bool:
    if VALIDATE_OFF:
        return True
    # TODO: enable EV/Trakt validation later using normalized meta+expected
    return True
...
def format_stream_inplace(
    ...
    # Right header: title (English if possible, else cleaned candidate)
    base_title = (expected.get("title") or "").strip()
    if not base_title:
        base_title = meta.get("title_raw") or "Unknown Title"
    base_title = _ui_clean(base_title, 62)
    year = expected.get("year")
    title_str = f"📄 {base_title}" + (f" ({year})" if year else "")
    s["title"] = _truncate(title_str, 92)
    ...
    # Always include a second line even if unknown
    bits = [f"📦 {size_str}", f"👥 {seeders if seeders>0 else '?'}"]
    if cached_hint:
        bits.insert(0, cached_hint.strip())
    line2 = " ".join(bits)
...
def process_streams(type_: str, id_: str, streams: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], PipeStats]:
    ...
    for s in streams:
        ...
        if not validate_stream(s, meta, expected):
            ...
            continue
        if REFORMAT_STREAMS:
            format_stream_inplace(s, meta, expected, cached_hint="")
        out.append(s)
    ...
    return jsonify(
        {
            ...
            "description": "Format-focused wrapper (2-left + 3-right) with validation pass-through",
            "resources": ["stream"],
            "types": ["movie", "series"],
...
def stream(type_: str, id_: str):
    try:
        ...
        return jsonify({"streams": [], "error": str(e)}), 200
